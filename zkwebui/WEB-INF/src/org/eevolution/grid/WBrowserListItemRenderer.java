...
public class WBrowserListItemRenderer implements ListitemRenderer, EventListener, ListitemRendererExt , ValueChangeListener
{
	...
	private String getDisplayText(Object value, GridField gridField)
	{
    	...
    	{
    		SimpleDateFormat dateFormat = DisplayType.getDateFormat(gridField.getDisplayType(),language);
    		return dateFormat.format((Timestamp)value);
    	}
    	else if (DisplayType.isNumeric(gridField.getDisplayType()))
    	{
    		return DisplayType.getNumberFormat(gridField.getDisplayType(),language).format(value);
    	}
	...
	public void valueChange(ValueChangeEvent evt) {
		...
		if (evt.getSource() instanceof WEditor) {
			...
			if (changedField != null) {
				...
				value = ((WEditor)evt.getSource()).getValue();

				row = getRowPosition(((WEditor)evt.getSource()).getComponent());
				col = getColumnPosition(((WEditor)evt.getSource()).getComponent());
			}
		} 
		else if (evt.getSource() instanceof Checkbox )
		{
			value = Boolean.valueOf(((Checkbox)evt.getSource()).isChecked());
			row = getRowPosition((Checkbox)evt.getSource());
			col = getColumnPosition((Checkbox)evt.getSource());
		}
		else if (evt.getSource() instanceof Decimalbox)
		{
			value = ((Decimalbox)evt.getSource()).getValue();
			row = getRowPosition((Decimalbox)evt.getSource());
			col = getColumnPosition((Decimalbox)evt.getSource());
		}
		else if (evt.getSource() instanceof Datebox)
		{
			value = ((Datebox)evt.getSource()).getValue();
			row = getRowPosition((Datebox)evt.getSource());
			col = getColumnPosition((Datebox)evt.getSource());
		}
		else if (evt.getSource() instanceof Textbox)
		{
			value = ((Textbox)evt.getSource()).getValue();
			row = getRowPosition((Textbox)evt.getSource());
			col = getColumnPosition((Textbox)evt.getSource());
		}
		
		if(value != null) {
			WTableColumn tableColumn = m_tableColumns.get(col);
			TableValueChangeEvent vcEvent = new TableValueChangeEvent(((WEditor)evt.getSource()).getComponent(),
					tableColumn.getHeaderValue().toString(),
					row, col,
					value, value);
